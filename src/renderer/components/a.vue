<template>
  <div>
    ...
    >
      <div class='box'>
        <button>Pulse</button>
        <button @click="handleCirclePosition">position</button>
...
export default {
  ...
  methods: {
    ...
    //   this.controllStage(false)
      this.layer.obj.draggable(false)
    },
    ...
    controllStage(type) {
      ...
      // 通过id 获取圆  从而获取圆的 x, y
      if (this.group.obj.find(`#${this.circleId}`)) {
        let circle = this.group.obj.find(`#${this.circleId}`)[0]
        this.circleArr.push({
            id: circle.attrs.id,
            x: circle.attrs.x,
            y: circle.attrs.y
    ...
    //   group.scaleY(Yscale)
      ...
      this.group.obj.width(width)
      this.group.obj.height(height)
      this.image.obj.width(width)
      this.image.obj.height(height)
